...
**Staged app.** The app is told the keyboard is remote. Its text-effects window, which is where an iPhone actually draws the keys and which is not in the app's window list, has every keyboard view forced out of the card on layout and again before the run loop sleeps. SpringBoard's client is the keyboard UI host, and SpringBoard's keyboard window is what shows the keys, full display width, above the card. The bottom card lifts. The stage window is not made key while an app is hosted. Both stages are marked staged, so the second app does this too. The app's own keys are not put back.
