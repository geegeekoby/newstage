...
**Staged app.** SpringBoard draws the keyboard outside the card by becoming the keyboard UI host. The app keeps the field that was tapped as the editor, so typing does not jump to a SpringBoard proxy field. Local key chrome inside the card is removed. Picker search is a different path and is unchanged. The in-app side loads only into Messenger (ElleKit ignores Exclude, so a UIKit-wide filter never reaches Messenger and would hit PaperBoard). The card the keys cover slides up. The other card stays on its half. Neither card changes size. Keyboard windows are not raised to alert level and no keyboard scene is placed.
