...
**Staged app.** Top and bottom keep the text field inside the app. Picker search is a different field and is not used for a hosted app. The app does not draw its own keys inside the card. Tapping a text field leaves that field as the editor, and SpringBoard draws the remote keyboard at the bottom of the display, outside the card. The stage window does not take the key for that keyboard. The card the keys cover slides up. The other card stays on its half. Neither card changes size. Closing the field hides the keyboard. Minimizing the stage or leaving the app hides it. The log records `keeps the message field` and, when the in-app side is running, `app: loaded` and `app: remote keyboard`. It also records whether that app loaded the stage dylib. The same lines are appended to `/var/mobile/Library/Preferences/com.recreated.dynamicstage.log`.
...
The arbiter is hooked only if `_UIKeyboardArbiter` is already loaded. The hook reads whether a keyboard changed. It does not place a keyboard scene and it does not choose a keyboard host. A staged app keeps its text field, and SpringBoard draws the remote keyboard.
