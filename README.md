...
**Staged app.** Top and bottom each have their own SpringBoard text field, outside the card. Picker search is a different field and is not used for a hosted app. The app does not draw its own keys, and it does not ask the keyboard arbiter for a keyboard. Tapping a text field makes the stage window the real key window, the same way search does, and that app's SpringBoard field then edits. UIKit shows that keyboard. The card the keys cover slides up. The other card stays on its half. Neither card changes size. Typed characters go only to the app that owns that field, including after the stage window takes the key. A letter is taken from the text field's own edit callback, which is where the keyboard delivers it, and it is also sent on a notification named for that app so the other stage does not receive it. The app listens for that letter as soon as its process starts, and installs the message-box hooks once it is on the card. An app that is already open is also treated as staged when its scene is the half-screen card. If putting the letter in through the field does not change its text, the text is written directly and the field's delegate is told. Closing that field hides the keyboard. Hiding Messenger's own keyboard no longer resigns the message box, so the blue line stays there while this keyboard is up. While the stage is still open, the home screen taking the key does not move the input off that field. Minimizing the stage or leaving the app hides it. The app's own keyboard stays hidden. The log records `staged key insert len=` and, from SpringBoard, `app: loaded`, `app: is listening`, and `app: key insert` with the field kind and whether the text changed. It does not record the characters. The line at the top reports the app and `lift slot0` / `lift slot1`. The same lines are appended to `/var/mobile/Library/Preferences/com.recreated.dynamicstage.log`.
...
The arbiter is hooked only if `_UIKeyboardArbiter` is already loaded. The hook reads whether a keyboard changed. It does not place a keyboard scene and it does not choose a keyboard host. A staged app is given its own SpringBoard keyboard instead.
