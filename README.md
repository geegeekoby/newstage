...
**Staged app.** Top and bottom keep the text field inside the app. Picker search is a different field and is not used for a hosted app. The in-app side loads into Messenger and into any app that links Foundation. Tapping a text field leaves that field as the editor. That side hides the keys the app drew inside the card and asks for SpringBoard's normal keyboard at the bottom of the display. The stage window does not take the key, and the keyboard window is not raised over the wallpaper. The card the keys cover slides up. The other card stays on its half. Neither card changes size. Closing the field hides the keyboard. Minimizing the stage or leaving the app hides it. The log records `keeps the message field` and, when the in-app side is running, `app: loaded` and `app: remote keyboard`. It also records whether that app loaded the stage dylib, and where the app dylib file sits. The same lines are appended to `/var/mobile/Library/Preferences/com.recreated.dynamicstage.log`.
...
The arbiter is hooked only if `_UIKeyboardArbiter` is already loaded. The hook reads whether a keyboard changed. It does not place a keyboard scene, does not take the key window, and does not change who draws the keyboard. A staged app keeps its text field. Its own dylib hides the keys inside the card.
