...
**Picker search.** The field is SpringBoard's, on every picker. The stage window becomes key only while a search field is editing. If an app is still staged when that edit ends, the window is handed back. The card that owns the field lifts above the keyboard (301pt floor). Opening a picker does not lift it. After a respring several scenes can be in the foreground at once. The stage window moves onto the scene of the window that actually has the key, takes that key, and then runs the same keyboard request. The diagnostics log keeps every line from that boot. `/var/mobile/Library/Preferences/com.recreated.dynamicstage.log` is replaced on every SpringBoard start. Each search attempt is written there and on the line at the top of the stage: window key, scene, first responder, and keyboard frame.

**Staged app.** Top and bottom both use that same picker search keyboard. The app does not draw its own keys, and it does not ask the keyboard arbiter for a keyboard. Tapping a text field makes the stage window the real key window, the same way search does, and a SpringBoard text field then edits. UIKit shows that keyboard. The card the keys cover slides up. The other card stays on its half. Neither card changes size. Typed characters go into the field that was tapped, including after the stage window takes the key. A letter is taken from the text field's own edit callback, which is where the keyboard delivers it, and it is also sent on the notification so an app that cannot read Preferences still receives it. The same letter is written under `/var/tmp` for apps that can read that path. The app listens for that letter with the same registration it uses to learn that it is on the stage. On this jailbreak ElleKit loads app tweaks from TweakInject, so install copies the in-app dylib there when that directory is real and restarts Messenger. An app that is already open is also treated as staged when its scene is the half-screen card, so the hooks are installed then rather than only at launch. If putting the letter in through the field does not change its text, the text is written directly and the field's delegate is told. Closing that field hides the keyboard. Hiding Messenger's own keyboard no longer resigns the message box, so the blue line stays there while the picker keyboard is up. While the stage is still open, the home screen taking the key does not move the input off that field. Minimizing the stage or leaving the app hides it. The app's own keyboard stays hidden. The log records `staged key insert len=` and, from SpringBoard, `app: key insert` with the field kind and whether the text changed. It does not record the characters. The line at the top reports the app and `lift slot0` / `lift slot1`. The same lines are appended to `/var/mobile/Library/Preferences/com.recreated.dynamicstage.log`.
...
The arbiter is hooked only if `_UIKeyboardArbiter` is already loaded. The hook reads whether a keyboard changed. It does not place a keyboard scene and it does not choose a keyboard host. A staged app is sent through the picker search keyboard instead.
