...
**Staged app.** Top and bottom keep the text field inside the app when the in-app side has loaded. If it has not, the same SpringBoard keyboard the picker uses is shown outside the card. Picker search is a different field. The stage window does not leave a black backdrop over the wallpaper. The card the keys cover slides up. Closing the field hides the keyboard. Minimizing the stage or leaving the app hides it. The log records whether the app loaded the stage dylib. Only SpringBoard writes that log.
...
The arbiter is hooked only if `_UIKeyboardArbiter` is already loaded. The hook reads whether a keyboard changed. It does not place a keyboard scene, does not take the key window, and does not change who draws the keyboard.
