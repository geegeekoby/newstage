...
**Staged app.** The app is told the keyboard is remote, and keyboard views in its text-effects window are forced out of the card. SpringBoard is asked to become the keyboard UI host. On this iPhone firmware SpringBoard often has no keyboard scene of its own, so hiding the app keyboard can leave no keys at all. A line at the top of the screen reports what happened: whether the app was staged, how many keyboard views were removed, and whether SpringBoard had a keyboard client and a keyboard scene. The same lines are appended to `/var/mobile/Library/Preferences/com.recreated.dynamicstage.log`.
