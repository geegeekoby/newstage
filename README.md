...
**Staged app.** Top and bottom keep the text field inside the app. Picker search is a different field and is not used for a hosted app. The in-app side loads into Messenger and into any app with UIApplication. Tapping a text field leaves that field as the editor. SpringBoard is the keyboard host and draws the keys at the bottom of the display, outside the card. The stage window does not take the key for that keyboard. The card the keys cover slides up. The other card stays on its half. Neither card changes size. Closing the field hides the keyboard. Minimizing the stage or leaving the app hides it. The log records `keeps the message field`, `keyboard ui host`, and, when the in-app side is running, `app: loaded` and `app: remote keyboard`. It also records whether that app loaded the stage dylib. The same lines are appended to `/var/mobile/Library/Preferences/com.recreated.dynamicstage.log`.
...
The arbiter is hooked only if `_UIKeyboardArbiter` is already loaded. The hook reads whether a keyboard changed. When the keyboard belongs to a staged app and picker search is not editing, SpringBoard is set as the keyboard UI host so the keys are drawn outside the card. The hook does not place a keyboard scene, does not take the key window, and does not change the text field that receives the letters.
